...
public class Funcionario extends Pessoa {
    ...
    public String gerarResumo() {
        return "Funcionario: " + getNome() + " | CPF: " + getCpf() + " | Salario: " + getSalario()
                + " | Ano Ingressado: " + getAnoIngresso();
    }

    public double calcularSalario(double salario) {

        int anoAtual = java.time.LocalDateTime.now().getYear();// metododo estatitico blibli. do java, pra pegar o
                                                               // horario java.time.localdate == uma hora(nulla). now()
                                                               // pega a dos istema e getyear so o ano
